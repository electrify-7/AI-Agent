...
AGENT_PROMPT_OUTBOUND_TEMPLATE = PromptTemplate.from_template(
    ...
    stage=6, if the customer is less interested, then offer discount found using calculate discount function. If the customer is still showing objections, gradually increase the discount from the initially offered one based on your descrition, it should always stay less than max_discount from the database.
...
stage=6, if the customer is less interested, then offer discount found using calculate discount function. If the customer still has objections or price concerns, keep looping in this stage.
stage=7, just focus on closing the deal and then politely end the call and do not repeat the things to the user.       
